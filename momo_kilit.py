...
def _bist_session_open() -> bool:
    # Render TR saatinde (genelde Türkiye)
    now = datetime.now()
...
def _default_kilit_state() -> dict:
    return {
        ...
        "history": {
            # "ALFAS": [{"ts": 1700000000, "close": 12.34, "vol": 123456, "pct": 1.2}, ...]
        }
    }
...
def _tv_scan_rows_for_symbols(symbols: List[str]) -> List[dict]:
    if not symbols:
        return []
    # TradingView tickers format: "BIST:ALFAS"
...
def _compute_kilit_score(samples: List[dict]) -> Tuple[int, Dict[str, str]]:
    ...
    Returns:
      score (0-100),
      tags dict (4 etiket)
    Not: etiketsel karar; mesajda sayısal metrik göstermiyoruz.
    """
    if len(samples) < 3:
        return 0, {
            "DEVAM": "YOK",
    ...
    # 1) Hacim devamlılığı (son örneklerin çoğu ortalamanın üstünde mi?)
    if mean_prev_vol <= 0.0:
        vol_cont_ratio = 0.0
    else:
        above = 0
        for v in vols[-5:]:
            if v >= mean_prev_vol * 0.9:
                above += 1
        vol_cont_ratio = above / float(min(5, len(vols)))
    ...
    dd = 0.0
    if peak > 0.0:
        dd = (peak - latest_close) / peak  # 0.00 = hiç çekilme yok, iyi
    # 4) Tutamama sinyali (hacim artarken fiyat tutunuyor / hızlanıyor)
    # Basit: vol_spike yüksek + pct_delta pozitif
    ...
    # Geri çekilme zayıflığı: 25
    # dd küçükse puan yüksek. dd 0% => 1.0, dd 1.2%+ => 0
    ...
    # Tutamama: 20
    # vol_spike 1.0 => 0, 1.8+ => 1.0 (ama pct_delta da lazım)
    vol_norm = 0.0
    if vol_spike > 1.0:
    ...
    if break_like:
        ...
        # alternatif: hacim artıyor ama ivme yoksa "TOPLUYOR"
...
def _score_badge(score: int) -> str:
    # Sayısal yok -> sadece seviye metni
    if score >= 80:
        return "ÇOK YÜKSEK"
    if score >= 70:
        return "YÜKSEK"
...
def _format_kilit_message(ticker: str, score: int, tags: Dict[str, str]) -> str:
    ...
    msg = (
        ...
        + "\n".join(tag_lines) +
        "\n\n"
        "🧠 <i>Mentor notu:</i> Tahta tutamama ihtimali yükseldi. Disiplinli takip.\n"
